...
export default class MusicEdit extends Vue {
  ...
  editRules: any = {
    ...
    musicUrl: [{ required: true, message: "请上传MP3", trigger: "blur" }],
    musicPic: [
  ...
  handleAvatarErrorMp3(err: any, file: any, fileList: any) {
    console.log("---> err", err);
  }
  ...
  handleAvatarSuccessMp3(res: any, file: any) {
    if (res.code === 200) {
  ...
  fileListDelete(file: any, fileList: any) {
    this.editForm.musicUrl = "";
  }
  beforeAvatarUploadMp3(file: any) {
  ...
  getDetail(id: any) {
    FindByIdApi({ musicId: id }).then((res: any) => {
      if (res.code == 200) {
